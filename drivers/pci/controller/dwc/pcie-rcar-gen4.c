 ...
 *
 * The r8a779g0 (R-Car V4H) requires a specific firmware to be provided,
 * to initialize the PHY. Otherwise, the PCIe controller will not work.
 *
 * The firmware file "104_PCIe_fw_addr_data_ver1.05.txt" is included as part of
 * the datasheet as a text file. However, Renesas is not able to distribute the
 * firmware freely. So, we require licensed users to convert the text file to
 * a binary before the driver runs by using the following command:
 *
 * $ awk '/^\s*0x[0-9A-Fa-f]{4}\s+0x[0-9A-Fa-f]{4}/ { print substr($2,5,2) substr($2,3,2) }' 104_PCIe_fw_addr_data_ver1.05.txt | xxd -p -r > rcar_gen4_pcie.bin
